---
name: real-work
description: Turn planning into a durable, resumable artifact for multi-step or multi-session work. Use when planning work that may outlive the current session — first reach complete understanding of scope by asking questions until no ambiguity remains, then write a phased plan to a descriptively named file in the repo root's plans folder, one that any future agent can resume from with zero prior context. Skip for trivial single-session tasks.
---
